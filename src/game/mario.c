...
f32 vec3f_find_ceil(Vec3f pos, f32 height, struct Surface **ceil) {
    ...
    return find_ceil(pos[0], height + 150.0f, pos[2], ceil); // height fix
}
...
void update_mario_geometry_inputs(struct MarioState *m) {
    ...
    m->waterLevel = find_water_level(m->pos[0], m->pos[2]) - 80; //water height
...
void mario_update_hitbox_and_cap_model(struct MarioState *m) {
    ...
    if (m->action & ACT_FLAG_SHORT_HITBOX) {
        m->marioObj->hitboxHeight = 10.0f;
    } else {
        m->marioObj->hitboxHeight = 180.0f;
    }
