...
s32 set_pole_position(struct MarioState *m, f32 offsetY) {
    ...
    if (m->pos[1] > ceilHeight - 250.0f) {
        m->pos[1] = ceilHeight - 250.0f;
...
s32 perform_hanging_step(struct MarioState *m, Vec3f nextPos) {
    ...
    if (ceilHeight - floorHeight <= 250.0f) {
        return HANG_HIT_CEIL_OR_OOB;
    }
    ...
    ceilOffset = ceilHeight - (nextPos[1] + 250.0f);
    if (ceilOffset < -30.0f) {
        return HANG_HIT_CEIL_OR_OOB;
    }
    if (ceilOffset > 30.0f) {
...
s32 act_ledge_grab(struct MarioState *m) {
    ...
    s32 hasSpaceForMario = (m->ceilHeight - m->floorHeight >= 250.0f);
